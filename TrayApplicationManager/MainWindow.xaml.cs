...
namespace TrayApplicationManager
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private void Setting_MouseEnter(object sender, RoutedEventArgs e)
        {
            ...
            else if (sender.Equals(this.ProcessPathTextBlock) || sender.Equals(this.ProcessPathTextBox))
            {
                HoverTB.Inlines.Add("The path of the process to start, leave empty to start any system process.");
            }
        ...
        private void SaveButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            var processName = Properties.Settings.Default.ProcessName;
            if (string.IsNullOrEmpty(processName))
            {
                MessageBox.Show("Please enter process name.");
                return;
            }
            Properties.Settings.Default.ProcessName = ProcessNameTextBox.Text;
            Properties.Settings.Default.ProcessPath = ProcessPathTextBox.Text;
            SaveSettings();
            if(CurrentStatus == ProcessStatus.Stopped || CurrentStatus == ProcessStatus.Checking || CurrentStatus == ProcessStatus.ProgramPaused)
            {
                var path = Properties.Settings.Default.ProcessPath.TrimEnd('\\');
                if (string.IsNullOrEmpty(path))
                {
                    try
                    {
                        Process.Start(Properties.Settings.Default.ProcessName);
                        SetProcessStatus(ProcessStatus.Running);
                    }
                    catch (Exception ex)
                    {
                        SetProcessStatus(ProcessStatus.Stopped);
                        MessageBox.Show("Couldn't start process. Unknown error occurred.");
                        return;
                    }
                ...
                {
                    var processPath = string.Format("{0}\\{1}", path, processName);
